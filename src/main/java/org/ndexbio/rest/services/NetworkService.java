...
public class NetworkService extends NdexService {
		...
		try {
			...
			final List<ODocument> adminCount = _ndexDatabase
					.query(new OSQLSynchQuery<Integer>(
							"select count(*) from Membership where in_members = ? and permissions = 'ADMIN'"));
	...
	@Path("/search")
	@Produces("application/json")
	public List<Network> findNetworks(SearchParameters searchParameters)
			throws IllegalArgumentException, NdexException {
	...
	public void updateNetwork(final Network updatedNetwork)
		...
		try {
			...
			if (updatedNetwork.getMetaterms() != null
					&& !updatedNetwork.getMetaterms().equals(
							networkToUpdate.getMetaterms())) {
				for (Entry<String, BaseTerm> metaterm : updatedNetwork
						.getMetaterms().entrySet()) {
					final String query = "SELECT FROM BaseTerm WHERE in_networkTerms = "
							+ networkToUpdate.asVertex().getId()
							+ " AND name = '"
							+ metaterm.getValue().getName()
							+ "'";
					...
					if (matchingTerms.size() > 0)
						networkToUpdate.addMetaterm(metaterm.getKey(),
								_orientDbGraph.getVertex(matchingTerms.get(0),
										IBaseTerm.class));
				}
			}
	...
	public void uploadNetwork(@MultipartForm UploadedFile uploadedNetwork)
			throws IllegalArgumentException, SecurityException, NdexException {
		_logger.info("uploadNetwork invoked");
		if (uploadedNetwork == null || uploadedNetwork.getFileData().length < 1) {
			_logger.error("No uploaded network");
		...
		_logger.info("upload file path is " +uploadedNetworkPath.getAbsolutePath());
		...
		try {
			...
			if (uploadedNetwork.getFilename().endsWith(".sif")
					|| uploadedNetwork.getFilename().endsWith(".xbel")
					|| uploadedNetwork.getFilename().endsWith(".xls")
					|| uploadedNetwork.getFilename().endsWith(".xlsx")) {
			...
			if (term instanceof IFunctionTerm) {
				...
				for (ITerm iterm : ((IFunctionTerm) term).getTermParameters()) {
					if (iterm instanceof IFunctionTerm)
						addTermAndFunctionalDependencies((IFunctionTerm) iterm,
								terms);
				}
			}
		}
	...
	private String buildSearchQuery(SearchParameters searchParameters) {
		...
		if (this.getLoggedInUser() != null) {
			query += "WHERE (isPublic = true"
					+ "\n  AND out_networkMemberships.out_membershipMember.username = '"
	...
	private void createEdges(final INetwork newNetwork,
		...
		for (final Entry<String, Edge> edgeEntry : networkToCreate.getEdges()
			...
			final IBaseTerm predicateTerm = (IBaseTerm) networkIndex
					.get(edgeToCreate.getPredicateId());
	...
	private void createSupports(final INetwork newNetwork,
		...
		for (final Entry<String, Support> supportEntry : networkToCreate
			...
			if (support.getCitationId() != null
				...
				newSupport.setSupportCitation((ICitation) networkIndex
						.get(support.getCitationId()));
	...
	private ArrayList<MetaParameter> parseMetaParameters(
			SearchParameters searchParameters, Pattern metaRegex) {
		...
		for (int groupIndex = 0; groupIndex < metadataMatches.groupCount(); groupIndex += 4) {
			metadataParameters.add(new MetaParameter(metadataMatches
					...
					groupIndex + 2).charAt(0), metadataMatches
					.group(groupIndex + 3)));
