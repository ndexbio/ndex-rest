...
public class TaskService extends NdexService {
	...
	public void deleteTask(@PathParam("taskId") String taskJid)
			throws Exception {
	...
	@Path("/tasks/{taskId}")
	@Produces("application/json")
	public Task getTask(@PathParam("taskId") String taskJid) throws NdexException {
		final ORID taskId = RidConverter.convertToRid(taskJid);
		final ITask task = _orientDbGraph.getVertex(taskId,
				ITask.class);
		...
		if (task == null) {
			final Collection<ODocument> matchingtasks = _orientDbGraph
					...
					.command(
							new OCommandSQL(
									"select from xtask where taskname = ?"))
					.execute(taskJid);

			if (matchingtasks.size() < 1)
				return null;
			else
				return new Task(_orientDbGraph.getVertex(
						matchingtasks.toArray()[0], ITask.class));
		} else
    ...
    public void updateTask(Task updatedTask) throws Exception
    {
	 ...
	 @PUT
	    @Produces("application/json")
	    public Task createTask(String ownerId, Task newTask) throws Exception
	    {
	        ...
	        try
	        {
	            final ITask task = _orientDbGraph.addVertex("class:xTask", ITask.class);
