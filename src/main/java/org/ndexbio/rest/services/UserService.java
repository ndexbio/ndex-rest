...
public class UserService extends NdexService
{
    ...
    **************************************************************************/
    ...
    public void changeProfileImage(@PathParam("imageType")final String imageType, @MultipartForm UploadedFile uploadedImage) throws IllegalArgumentException, NdexException
    {
        ...
        try
        {
            ...
            {
                final BufferedImage resizedImage = resizeImage(newImage, Integer.parseInt(Configuration.getInstance().getProperty("Profile-Image-Width")), Integer.parseInt(Configuration.getInstance().getProperty("Profile-Image-Height")));

                ImageIO.write(resizedImage, "jpg", new File(Configuration.getInstance().getProperty("Profile-Image-Path") + user.getUsername() + ".jpg"));
            }
            else
            {
                final BufferedImage resizedImage = resizeImage(newImage, Integer.parseInt(Configuration.getInstance().getProperty("Profile-Background-Width")), Integer.parseInt(Configuration.getInstance().getProperty("Profile-Background-Height")));
    ...
    **************************************************************************/
    ...
    public Response emailNewPassword(@PathParam("username")final String username) throws IllegalArgumentException, NdexException
    {
        ...
        try
        {
            ...
            final Collection<ODocument> usersFound = _ndexDatabase.command(new OCommandSQL("select from User where username = ?")).execute(username);
            ...
            Email.sendEmail(Configuration.getInstance().getProperty("Forgot-Password-Email"), authUser.getEmailAddress(), "Password Recovery", forgotPasswordText.toString());
    ...
    **************************************************************************/
    ...
    @Produces("application/json")
    public List<User> findUsers(SearchParameters searchParameters, @PathParam("searchOperator")final String searchOperator) throws IllegalArgumentException, NdexException
    {
        ...
        if (operator.equals("exact-match"))
        {
            ...
            query = "SELECT FROM User\n" + "WHERE username.toLowerCase() = '" + searchParameters.getSearchString() + "'\n" + "  OR lastName.toLowerCase() = '" + searchParameters.getSearchString() + "'\n" + "  OR firstName.toLowerCase() = '" + searchParameters.getSearchString() + "'\n" + "ORDER BY creation_date DESC\n" + "SKIP " + startIndex + "\n" + "LIMIT " + searchParameters.getTop();
        }
